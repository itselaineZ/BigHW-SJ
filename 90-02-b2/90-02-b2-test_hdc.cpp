...
#endif
	//hdc_draw_Mathematical_curve_1952339();
...
//			hdc_arc(350, 350, 200+i*10, t[i].angle_begin, t[i].angle_end, 2+rand()%5, RGB(rand() % 256, rand() % 256, rand() % 256));
	...
	if (test_choise[T_SECTOR] == '1') {
		...
		/* 先测试几个指定大小的，目的是为了验证角度计算的正确性 */
		for (round = 0; round <= 1; round ++)
			for (i = 8; t[i].angle_begin != -1 && t[i].angle_end != -1; i++) {
...
//				hdc_sector(350, 350, 200 - round * 20 + i * 10, t[i].angle_begin, t[i].angle_end, round, 2+rand()%5, RGB(rand() % 256, rand() % 256, rand() % 256)); //随机色(醒目程度不如预置三色轮转)
	...
	if (test_choise[T_PEPPAPIG] == '1') {
		...
		hdc_draw_PeppaPig(300, 500); //左边鞋子的左上角为基准坐标，尝试一下改变基准坐标
		}

	if (test_choise[T_MATHCURVE] == '1') {
		/* 画一条数学曲线 */
		hdc_draw_Mathematical_curve_1851441();
		to_be_continued("数学曲线-陈江浩哥哥-完成", STL_X, STL_Y);

		hdc_draw_Mathematical_curve_1851573();
		to_be_continued("数学曲线-李博宇哥哥-完成", STL_X, STL_Y);

		hdc_draw_Mathematical_curve_1851977();
		to_be_continued("数学曲线-李家麟哥哥-完成", STL_X, STL_Y);
	...
	if (test_choise[T_CUSTOM] == '1') {
		...
		hdc_draw_catoon_1851441(400, 300);
		to_be_continued("卡通人物-陈江浩哥哥-完成", STL_X, STL_Y);

		hdc_draw_catoon_1851573(350, 270);
		to_be_continued("卡通人物-李博宇哥哥-完成", STL_X, STL_Y);

		hdc_draw_catoon_1851977(300, 700);
		to_be_continued("卡通人物-李家麟哥哥-完成", STL_X, STL_Y);

		/* 在此添加自己的卡通人物，具体内容可自选，额外加分项 */
	}
	...
	cout << "time=" << (double(t_end) - double(t_start)) / 1000.0 << endl;
	system("pause");
#endif
	hdc_draw_Mathematical_curve_1952339();
	cout << "\n请任意按键继续\n";
	getchar();
	hdc_draw_catoon_1952339(300, 200);
	return 0;
}
