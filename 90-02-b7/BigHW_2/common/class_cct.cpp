...
/* 类的静态成员初始化（只读） */
const HANDLE CCT::__hout = GetStdHandle(STD_OUTPUT_HANDLE);	//取标准输出设备对应的句柄
const HANDLE CCT::__hin = GetStdHandle(STD_INPUT_HANDLE);		//取标准输入设备对应的句柄
...
/***************************************************************************
  ...
  说    明：颜色的取值由背景色+前景色组成，各16种
			...
			最终的颜色为 背景色*16+前景色
***************************************************************************/
void CCT::setcolor(const CCT_COLOR bg_color, const CCT_COLOR fg_color) const
{
	SetConsoleTextAttribute(__hout, int(bg_color) * 16 + (int)(fg_color));
//	SetConsoleTextAttribute(__hout, static_cast<int>(bg_color) * 16 + static_cast<int>(fg_color));
}
...
***************************************************************************/
void CCT::getcolor(int &bg_color, int &fg_color) const
{
...
/***************************************************************************
  ...
  说    明：屏幕左上角坐标为(0,0)，在cmd窗口的大小未被调整的情况下，Win10为：
			...
			纵向y轴，对应行(0-29)
***************************************************************************/
void CCT::gotoxy(const int X, const int Y) const
{
	COORD coord;
	coord.X = X;
	coord.Y = Y;
...
/***************************************************************************
  ...
  说    明：形参中的&表示引用，第六章会讲到，引用形参的值可以返回给实参
***************************************************************************/
void CCT::getxy(int &x, int &y) const
{
	CONSOLE_SCREEN_BUFFER_INFO binfo;
	GetConsoleScreenBufferInfo(__hout, &binfo);
...
/***************************************************************************
  ...
  说    明：
***************************************************************************/
void CCT::setcursor(const CCT_CURSOR options) const
{
	CONSOLE_CURSOR_INFO cursor_info;

	switch (options) {
		case CCT_CURSOR::full:
			cursor_info.bVisible = 1;
			cursor_info.dwSize = 100;
			break;
		case CCT_CURSOR::half:
			cursor_info.bVisible = 1;
			cursor_info.dwSize = 50;
			break;
		case CCT_CURSOR::invisible:
			cursor_info.bVisible = 0;
			cursor_info.dwSize = 1;
			break;
		case CCT_CURSOR::normal:
		default: //缺省显示光标，横线
			cursor_info.bVisible = 1;
...
***************************************************************************/
void CCT::showch(const int X, const int Y, const char ch, const CCT_COLOR bg_color, const CCT_COLOR fg_color, const int rpt) const
{
	int i;

	this->gotoxy(X, Y);
	this->setcolor(bg_color, fg_color);
...
/***************************************************************************
  ...
  说    明：
***************************************************************************/
void CCT::showstr(const int X, const int Y, const char *str, const CCT_COLOR bg_color, const CCT_COLOR fg_color, int rpt, int maxlen) const
{
#if 0
	const char *p;
	int i;
...
说    明：
***************************************************************************/
void CCT::showint(const int X, const int Y, const int num, const CCT_COLOR bg_color, const CCT_COLOR fg_color, const int rpt) const
{
	int i;
...
***************************************************************************/
void CCT::setconsoleborder(int set_cols, int set_lines, int set_buffer_cols, int set_buffer_lines) const
{
	/* 取当前系统允许的窗口的行列最大值 */
	COORD max_coord;
...
/***************************************************************************
  ...
  说    明：
***************************************************************************/
void CCT::getconsoleborder(int &cols, int &lines, int &buffer_cols, int &buffer_lines) const
{
	CONSOLE_SCREEN_BUFFER_INFO binfo;
	GetConsoleScreenBufferInfo(__hout, &binfo);
...
/***************************************************************************
  ...
  说    明：
***************************************************************************/
void CCT::getconsoletitle(char *title, int maxbuflen) const
{
...
/***************************************************************************
  ...
  说    明：
***************************************************************************/
void CCT::setconsoletitle(const char *title) const
{
...
***************************************************************************/
void CCT::enable_mouse(void) const
{
	DWORD Mode;
...
说    明：某些cmd窗口控制语句执行后，可能会取消鼠标支持，则调用本函数回再次加入
***************************************************************************/
void CCT::disable_mouse(void) const
{
	DWORD Mode;
...
/***************************************************************************
  ...
  说    明：下列说明来自鼠标定义文件
			...
			35.0                       当鼠标有键被按下或者释放
***************************************************************************/
CCT_KMEVENT CCT::read_keyboard_and_mouse(int &MX, int &MY, CCT_MOUSE&MAction, int& keycode1, int& keycode2) const
{
	static int MX_old = -1, MY_old = -1;
	CCT_MOUSE MAction_old = CCT_MOUSE::only_moved;
	...
	while (1) {
		...
		if (InputRec.EventType == KEY_EVENT) {
			...
			if (InputRec.Event.KeyEvent.bKeyDown) { // 只在按下时判断，弹起时不判断
				...
				switch (InputRec.Event.KeyEvent.wVirtualKeyCode) {
					case VK_UP:
						keycode1 = static_cast<int>(CCT_KEYCODE::arrow_pre);
						keycode2 = static_cast<int>(CCT_KEYCODE::arrow_up);		//模拟 _getch()方式返回的两个键码，分别是224（0xE0）和72（0x48）
						break;
					case VK_DOWN:
						keycode1 = static_cast<int>(CCT_KEYCODE::arrow_pre);
						keycode2 = static_cast<int>(CCT_KEYCODE::arrow_down);	//模拟 _getch()方式返回的两个键码，分别是224（0xE0）和80（0x50）
						break;
					case VK_LEFT:
						keycode1 = static_cast<int>(CCT_KEYCODE::arrow_pre);
						keycode2 = static_cast<int>(CCT_KEYCODE::arrow_left);	//模拟 _getch()方式返回的两个键码，分别是224（0xE0）和75（0x4B）
						break;
					case VK_RIGHT:
						keycode1 = static_cast<int>(CCT_KEYCODE::arrow_pre);
						keycode2 = static_cast<int>(CCT_KEYCODE::arrow_right);	//模拟 _getch()方式返回的两个键码，分别是224（0xE0）和77（0x4D）
						break;
					default:
						break;
				} //end of switch
		...
		if (InputRec.EventType == MOUSE_EVENT) {
			...
			if (InputRec.Event.MouseEvent.dwEventFlags == MOUSE_MOVED) {//鼠标移动
				...
				if (MX_old == MX && MY_old == MY && MAction_old == CCT_MOUSE::only_moved)
					continue;

				/* 位置变化则记录下来 */
				MX_old = MX;	
				MY_old = MY;
				MAction = CCT_MOUSE::only_moved;
				MAction_old = MAction;
				return CCT_KMEVENT::mouse;
			}
			MAction_old = CCT_MOUSE::no_action; //置非MOUSE_ONLY_MOVED值即可
			...
			/* https://docs.microsoft.com/en-us/windows/console/mouse-event-record-str
				...
				if (InputRec.Event.MouseEvent.dwButtonState & 0x80000000) //高位为1，负数
					MAction = CCT_MOUSE::wheel_moved_down;
				else
					MAction = CCT_MOUSE::wheel_moved_up;
				return CCT_KMEVENT::mouse;
			}

			if (InputRec.Event.MouseEvent.dwButtonState == (FROM_LEFT_1ST_BUTTON_PRESSED | RIGHTMOST_BUTTON_PRESSED)) { //同时按下左右键
				MAction = CCT_MOUSE::leftright_button_click;
				return CCT_KMEVENT::mouse;
			}
				...
				if (InputRec.Event.MouseEvent.dwEventFlags == DOUBLE_CLICK)
					MAction = CCT_MOUSE::left_button_doubleclick;
				else
					MAction = CCT_MOUSE::left_button_click;
				return CCT_KMEVENT::mouse;
			}
			else if (InputRec.Event.MouseEvent.dwButtonState == RIGHTMOST_BUTTON_PRESSED) { //按下右键
				if (InputRec.Event.MouseEvent.dwEventFlags == DOUBLE_CLICK)
					MAction = CCT_MOUSE::right_button_doubleclick;
				else
					MAction = CCT_MOUSE::right_button_click;
				return CCT_KMEVENT::mouse;
			}
			else if (InputRec.Event.MouseEvent.dwButtonState == FROM_LEFT_2ND_BUTTON_PRESSED) { //按下滚轮
				MAction = CCT_MOUSE::wheel_click;
				return CCT_KMEVENT::mouse;
			}
			else //忽略其它按键操作
				continue;
	...
	return CCT_KMEVENT::mouse; //此句应该执行不到，为避免某些编译器报不完全分支错误，加
}
...
/***************************************************************************
  ...
  说    明：
***************************************************************************/
int CCT::getfontinfo(void) const
{
...
/***************************************************************************
  ...
  说    明：
***************************************************************************/
void CCT::setconsolefont(const int font_no) const
{
	HMODULE hKernel32 = GetModuleHandleA("kernel32");
	if (hKernel32 == 0)
		return;
...
***************************************************************************/
void CCT::setfontsize(const char *fontname, const int high, const int width) const
{
